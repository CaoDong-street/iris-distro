...
double inner_ellipsoid(const iris::Polyhedron &polyhedron, iris::Ellipsoid *ellipsoid, MSKenv_t *existing_env)
{
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        check_res(MSK_putconbound(task, con_ndx, MSK_BK_FX, polyhedron.getB()(i, 0),
                                  polyhedron.getB()(i, 0))); 
        con_ndx++;
    }
    ...
    for (int j = 0; j < num_s; j++)
    {
        ...
        check_res(
            MSK_appendcone(task, MSK_CT_RQUAD, 0.0, 3, csub)); // 3rd argument (0.0) is reserved for future use by Mosek
        lhs_idx += 2;
    }
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        check_res(MSK_appendcone(task, MSK_CT_QUAD, 0.0, n + 1,
                                 csub_f_row.data())); // 3rd argument (0.0) is reserved for future use by Mosek
    }
    ...
    check_res(MSK_putbarablocktriplet(task, nabar, bara_i.data(), bara_j.data(), bara_k.data(), bara_l.data(),
                                      bara_v.data())); 
    ...
    switch (solsta)
    {
        ...
        case MSK_SOL_STA_UNKNOWN:
            ...
            throw(InnerEllipsoidInfeasibleError());
        default: printf("other solution status: %d\n", solsta); throw(InnerEllipsoidInfeasibleError());
    }
...
void closest_point_in_convex_hull(const MatrixXd &Points, VectorXd &result, MSKenv_t *existing_env)
{
    ...
    switch (solsta)
    {
        case MSK_SOL_STA_OPTIMAL:
        case MSK_SOL_STA_NEAR_OPTIMAL:
        case MSK_SOL_STA_UNKNOWN: MSK_getxxslice(task, MSK_SOL_ITR, 0, dim, result.data()); break;
        case MSK_SOL_STA_DUAL_INFEAS_CER:
        case MSK_SOL_STA_PRIM_INFEAS_CER:
        case MSK_SOL_STA_NEAR_DUAL_INFEAS_CER:
        case MSK_SOL_STA_NEAR_PRIM_INFEAS_CER:
            ...
            // std::cout << "Closest point in convex hull: The status of the solution could not be determined." <<
            // std::endl; if (!existing_env) {
            //   MSK_deleteenv(env);
            //   free(env);
            // }
            // throw(InnerEllipsoidInfeasibleError());
